...
class Mastodon(SocialPoster):
    ...
    def post(self, text):
        ...
        if response.status_code != 200:
            print("Failed to post to mastodon: {}".format(response.text))
            return None
    ...
    def set_pin(self, postid):
        ...
        if response.status_code != 200:
            print("Failed to get mastodon credentials: {}".format(response.text))
            return False

        # Next, get the currently pinned ones.
        response = requests.get(
            '{}/api/v1/accounts/{}/statuses'.format(self.settings.MASTODON_BASEURL, response.json()['id']),
            headers={'Authorization': 'Bearer {}'.format(self.settings.MASTODON_TOKEN)},
            params={'pinned': 'true'},
            timeout=10,
        )
        if response.status_code != 200:
            print("Failed to get list of mastodon pins: {}".format(response.text))
            return False

        found = False
        for pinned in response.json():
            if pinned['id'] == postid:
                # Already pinned!
                found = True
            else:
                ...
                unpin_response = requests.post(
                    '{}/api/v1/statuses/{}/unpin'.format(self.settings.MASTODON_BASEURL, pinned['id']),
                    headers={'Authorization': 'Bearer {}'.format(self.settings.MASTODON_TOKEN)},
                    timeout=10,
                )
                if unpin_response.status_code != 200:
                    print("Failed to unpin from mastodon: {}".format(unpin_response.text))
        ...
        if not found and postid is not None:
            ...
            if pin_response.status_code != 200:
                print("Failed to pin to mastodon: {}".format(pin_response.text))
                return False
    ...
    def register(self, clientname):
        ...
        if (
            ...
            response = requests.post('{}/api/v1/apps'.format(baseurl), {
        ...
        if getattr(self.settings, 'MASTODON_TOKEN', None) is None:
            session = requests_oauthlib.OAuth2Session(
                ...
                scope='read write:statuses write:media write:accounts',
            )
...
class Bluesky(SocialPoster):
    ...
    def _get_token_and_repo(self):
        response = requests.post('https://bsky.social/xrpc/com.atproto.server.createSession', json={
            'identifier': self.settings.BLUESKY_USER,
            'password': self.settings.BLUESKY_PASSWORD,
        }, timeout=10)
        response.raise_for_status()
        self._token = response.json()['accessJwt']
        self._repo = response.json()['did']
    ...
    def post(self, text):
        ...
        if response.status_code == 400 and response.headers.get('content-type', '').startswith('application/json'):
            payload = response.json()
            print("Failed to post to bluesky: {}, message {}".format(payload['error'], payload['message']))
            return None
        if response.status_code != 200:
            print("Failed to post to bluesky: {}".format(response.text))
            return None
    ...
    def set_pin(self, postid):
        ...
        if response.status_code != 200:
            print("Failed to get bluesky profile: {}".format(response.text))
            return False
        ...
        if postid:
            ...
            if post_response.status_code != 200:
                print("Failed to read back bluesky post {}: {}".format(postid, post_response.text))
                return False

            cid = post_response.json()['posts'][0]['cid']

            record['pinnedPost'] = {
                'uri': postid,
                'cid': cid,
            }
        elif 'pinnedPost' in record:
            del record['pinnedPost']

        if record != response.json()['value']:
            ...
            if save_response.status_code != 200:
                print("Failed to save back bluesky profile for pinned posts: {}".format(save_response.text))
                return False
            return True
    ...
    def register(self, clientname):
        if getattr(self.settings, 'BLUESKY_USER', None) is None or getattr(self.settings, 'BLUESKY_PASSWORD', None) is None:
            return (
                "For bluesky, add an 'app password' from 'Settings' -> 'Privacy and Security'.\n"
    ...
    _facet_parsers = (
        (
            ...
            'uri',
            re.compile(rb"\s+(https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*[-a-zA-Z0-9@%_\+~#//=])?)"),
            0,
            0,
        ),
    ...
    def _parse_facets(self, text: bytes):
        """
        Parse post text and return app.bsky.richtext.facet objects for any URLs or hashtags.

        Indexing must work with UTF-8 encoded bytestring offsets, not regular unicode string offsets,
        to match Bluesky API expectations.
        """
        for facet_type, field, regex, startofs, endofs in self._facet_parsers:
            for match in regex.finditer(text):
                yield {
                    "index": {
                        "byteStart": match.start(1) + startofs,
                        "byteEnd": match.end(1) + endofs,
                    },
                    "features": [{"$type": facet_type, field: match.group(1).decode("UTF-8")}],
                }
...
def get_all_providers(settings, returnallproviders=False):
    ...
    return allproviders, [provider.name for provider in allproviders]
