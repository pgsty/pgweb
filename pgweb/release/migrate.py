...
def do_migrate():
    ...
    if (release['type'] in ('major', 'override') and
            not release.get('announcetext') and
            not release.get('announcetext_zh')):
        raise Exception("Release type {} must specify announcetext_zh or announcetext".format(release['type']))
    ...
    if news:
        for cve in release.get('cve', []):
            try:
                ...
                print("Could not find CVE {}, cannot connect to announcement".format(cve))
    ...
    if news:
        if news.modstate != ModerationState.APPROVED:
            ...
            if settings.RELEASE_SEND_ANNOUNCEMENT_EMAIL:
                print("Sending announcement email for news item")
                news.on_approval(None)
            else:
                print("Announcement email disabled; not sending news item")

        pa = PinnedNewsArticle.objects.first()
        if pa is None:
            print("Creating pinned news record for news item {}".format(news.id))
            PinnedNewsArticle.objects.create(pinnedarticle=news)
        elif pa.pinnedarticle != news:
