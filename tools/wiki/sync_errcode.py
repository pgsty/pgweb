#!/usr/bin/env python3
"""把 pgsty/err.pg.center 的错误码大全同步进本地或生产 PGWeb 库。

导出与导入分两步，好让同一份快照分别加载两端；生产机上不需要有源仓库。
默认只预览不写库。
"""
...
def main():
    ...
    parser.add_argument('--root', default='~/pg.center/err', help='源仓库路径')
    ...
    try:
        ...
        if args.export:
            ...
            print('已导出 {} 个错误码到 {}'.format(len(snapshot['codes']), args.export))
            return 0

        if args.target == 'production':
            remote = ['.venv/bin/python', 'tools/wiki/sync_errcode.py', '--input', '-']
            if args.write:
                remote.append('--write')
            if args.prune:
                remote.append('--prune')
            ...
            report = importer.import_snapshot(snapshot, prune=args.prune)
            errcode.forget()
        else:
