...
同一物理提交也可能包含多个独立事项。检测到同次发布、同一原始分区的不同完整正文共享某个提交组时，涉及该组的等价与扣除都要求完整英文正文相同。例如 16.2 的大对象权限检查修复不会被旧分支已有的 PostAlterHook 报告修复掩盖；9.3.3 中分属两个 CVE 的缓冲区与 crypt 修复仍分别保留。迁移分区按原始来源坐标识别，即使条目同时归入安全类别也不与普通变更混合。
