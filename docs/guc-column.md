...
- **索引**：面包屑 → H1「PostgreSQL 配置参数」→ 导语 → 十六个分类入口（`wiki-classnav` 形，紧凑网格，中文名 + 数量 + 英文眉题）→ 版本条（18 个药丸链到变更页，实心 = 默认版本，虚线 = 未发布）→ 搜索框 + 四个下拉 → 图例 → 分组大表 → 说明段。
  ...
  版本变动是一排方格（一版一格，颜色按 state，`title` 写 label），表头下方的横向刻度与之逐格对齐，每一格都写出版本号（9.x 的点号单独缩小，占位与两位数相当，由 `mark_ticks()` 拆成 `tick_head` / `tick_tail`）。版本号按支持状态着色，四档由 `pgweb/wiki/ruler.py` 的 `tone_of()` 从 `support_status` 推出：橙 = 已停止维护、绿 = 仍在支持、蓝 = 测试版、紫 = 开发版；同一套色也用在页面上方的版本导航条上（模板给药丸加 `wiki-vpill is-<tone>`）。
