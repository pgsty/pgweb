...
| 形态 | 条数 | 处理 |
| --- | --- | --- |
| `../25p02/` 站内码链接 | 1400 | → `/docs/sqlstate/25P02/`，slug 在源里是小写，站内统一大写 |
...
/docs/sqlstate/                  错误码大全索引
/docs/sqlstate/23505/            详情页
/docs/sqlstate/class/23/         Class 23 及其全部成员
/docs/sqlstate/version/18/       PG 18 的全部错误码，含与 17 的增删对比
/docs/sqlstate/message/          报文反查
```
...
`/docs/sqlstate/version/18/` 列出该版本全部错误码，并给出与上一版本的增删对比。
...
- 手册 → 百科：给附录页加一个只在该页加载的外部 JS，把表格里的 `<code class="literal">23505</code>`
  变成指向 `/docs/sqlstate/23505/` 的链接。不改手册正文 HTML，可随时撤下。
...
```
/docs/sqlstate/            索引：导航索引 + 按类分组的大表格 + 即时筛选
/docs/sqlstate/23505/      详情，小写地址 301 到大写
/docs/sqlstate/23505/?v=16 切换本站手册链接的版本
```
...
- **上游链接对本站独有栏目会 404**。`source_url` 把任何路径前缀成 `postgresql.org/<path>`，
  而 `/docs/sqlstate/`、`/info/`、`/ext/`、`/e/` 上游都没有对应页。已在 `pgweb/util/contexts.py` 里按前缀抑制该链接。
