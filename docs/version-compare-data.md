...
来源审计证明按原文无损抽取，不证明每组上游注释都表达同一事项。[9.3.3 的提交](https://github.com/postgres/postgres/commit/01824385a)明确合并了两个独立安全修复；[16.2 的提交](https://github.com/postgres/postgres/commit/59bd34c2f)同时包含只影响新分支与影响旧分支的不同事项。最新 14–18 的 `to_char()` 条目还保留了疑似混入的备库重连注释，实际提交修改的文件与其声明事项不一致。本站保留原始注释作为来源证据，利用同发布多正文冲突标记歧义提交，不据此自动抹去独立变更。关系规则见 [存储契约](version-compare-storage.md)。
