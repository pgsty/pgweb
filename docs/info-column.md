...
入口：顶部导航「博览」放在最后（「支持」之后），下拉为 最新收录 / 每日更新 / 归档 / 搜索；在博览页面按 `/` 聚焦博览搜索框而不是文档检索弹窗（⌘K 仍打开弹窗）；页脚不放「博览」；首页在「遇到异常行为」区块之后放一个「博览」区块：最近一天的全部条目按阅读顺序排成两栏紧凑列表（不分档、无配图、无摘要），标题外链原文，有摘要的条目带「详情」锚点链接到当天页面，下方是「查看当天详情」与「更多博览」。搜索页、每日页、归档页对搜索引擎 `noindex`（信息流首页与每日页可收录）。主站爬虫（`tools/search/crawler`）跳过 `/info/` 路径。
