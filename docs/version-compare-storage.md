...
```json
{"target":"<32 hex entry ID>","type":"equivalent","rule":1,"evidence":{"patch_ids":["<patch ID>"],"same_day":true}}
```
...
事务内逐行哈希避免无意义更新，然后严格从数据库重建原始 JSON，验证数量、所有未知字段、完整原始对象相等及整体哈希，最后原子激活 manifest。失败整体回滚。无变化的重复导入不改数据修订、正文历史或写入时间。
