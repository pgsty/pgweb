...
第三方组件只提供外部 URL 入口，不在 pg.center 导入、保存或渲染文档正文，也不纳入本站文档检索。文档页面左侧的“三方文档”默认收起；顶部“文档”下拉菜单与移动端文档导航的最下面也提供“三方文档”入口，链接到 `/docs/third-party/`，不在菜单中展开组件子项。`/docs/` 首页正文不重复展示。
