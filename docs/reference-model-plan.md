...
本文为已获授权执行的原始计划，实际进展与验收见 [实施记录](reference-model-rollout.md)。下文保留规划时的研究基线：研究基线是本地仓库 `c0aaae4d`、本地 `pgweb` 数据库；本轮未修改应用代码、迁移或数据库，未核验生产结构。下列迁移名、待新增模块和命令均为规划，不能视为已经存在或执行。
...
以上为未来实施入口，本轮未执行。`tools/wiki/check_data.sql` 改造后分别在正确的本地连接与 `service=pgweb.pg` 检查；SQLSTATE 按自己的实际覆盖范围校验，不套用其他领域“PG10–20 必须齐全”的规则。
