...
function versionLine(data, onVersion) {
  ...
  data.versions.forEach((item, index) => {
    ...
    link.addEventListener('click', (event) => {
      ...
      if (data.source === 'errcode' || data.source === 'catalog' || data.source === 'guc') return;
      event.preventDefault();
      onVersion(item.id);
    });
    line.append(link);
...
export function renderPreview(container, data, options = {}) {
  ...
  bar.append(kind);
  const open = el('a', 'ds-preview-open', data.source === 'ext' ? '打开扩展页' : (data.source === 'errcode' || data.source === 'catalog' || data.source === 'guc') ? '打开词条' : '打开文档');
  open.href = data.url;
